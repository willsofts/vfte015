    ...
    <template v-slot:default>
        <div class="row row-height">
          <div class="col-height col-md-4">
            <label for="msgcode">{{ labels.msgcode_label }}</label>
            <div class="input-group has-validation" :class="{'has-error': v$.msgcode.$error}">
              <InputMask ref="msgcode" v-model="localData.msgcode" id="msgcode" name="msgcode" picture="(50)X" :disabled="disabledKeyField"/> 
              <label class="required">*</label>
            </div>
            <span v-if="v$.msgcode.$error" class="has-error">{{ v$.msgcode.$errors[0].$message }}</span>
          </div>
        </div>
        <div class="row row-height">
          <div class="col-height col-md-4">
            <label for="langcode">{{ labels.langcode_label }}</label>
              <fieldset id="langcodefieldset" :disabled="disabledKeyField"><legend class="fa-hidden"></legend>
                <div class="input-group has-validation" :class="{'has-error': v$.langcode.$error}">
                  <select ref="langcode" id="langcode" v-model="localData.langcode" class="form-control input-md">
        ...
        <div class="row row-height">
          <div class="col-height col-md-11">
            <label for="msgtext">{{ labels.msgtext_label }}</label>
            <div class="input-group has-validation" :class="{'has-error': v$.msgtext.$error}">
              <input type="text" ref="msgtext" v-model="localData.msgtext" id="msgtext" class="form-control input-md" maxlength="200" /> 
              <label class="required">*</label>
            </div>
            <span v-if="v$.msgtext.$error" class="has-error">{{ v$.msgtext.$errors[0].$message }}</span>
          </div>
        </div>
    </template>
...
export default {
  ...
  setup(props) {
    ...
    const validateRules = computed(() => { 
      return {
        msgcode: { required: requiredMessage() },
        langcode: { required: requiredMessage() },
        msgtext: { required: requiredMessage() },
      } 
    });
  ...
  methods: {
    ...
    startInsertRecord() {
      this.resetRecord();
      this.showDialog(() => { this.$refs.msgcode.focus(); });
    },
    startSaveRecord() {
      confirmSave(() => {
        ...
        $.ajax({
          ...
          success: (data) => {
            ...
            successbox(() => {
              ...
              this.resetRecord();
              setTimeout(() => { this.$refs.msgcode.focus(); },100);
      ...
      $.ajax({
        ...
        success: (data) => {
          ...
          if(data.body.dataset) {
            ...
            this.showDialog(() => { this.$refs.msgtext.focus(); });
          }
        }
      });	
    },
